...
NUMERIC_COLUMNS_2DP = {
    "Position_m", "FX_kN", "FZ_kN", "MY_kNm",
    "Total_Length_m", "Unit_Mass_kg_m", "Total_Weight_kg",
}
...
class ExcelReporter:
    ...
    def _build_sheet(
        ...
        banner_title: str,
        df: pd.DataFrame,
        summary_rules: Optional[list] = None,
        use_excel_sum_formula: bool = False,
    ) -> Worksheet:
        ...
        if summary_rules:
            ...
            for label_col_name, agg, label in summary_rules:
                ...
                if use_excel_sum_formula and agg == "sum":
                    value_cell.value = f"=SUM({col_letter}{first_data_row}:{col_letter}{last_data_row})"
                elif agg == "sum":
                    value_cell.value = float(df[label_col_name].sum())
                elif agg == "max":
                    value_cell.value = f"=MAX({col_letter}{first_data_row}:{col_letter}{last_data_row})"
                elif agg == "min":
                    value_cell.value = f"=MIN({col_letter}{first_data_row}:{col_letter}{last_data_row})"
