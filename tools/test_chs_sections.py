...
from tools.section_data import has_full_dims, shape_kind
from tools.section_sizing import available_sections, section_depth_mm, section_families
...
def chs_props(D_mm, t_mm, fy_mpa=None):
    ...
    return {
        "shape_type": 36,
        "shape_kind": "circular_hollow",
        "complete": True,
        "area_m2": A,
        "iy_m4": math.pi / 64.0 * (D**4 - (D - 2 * t) ** 4),
        ...
        "tf_m": t,
        "r_m": 0.0,
        "wy_m3": math.pi / 32.0 * (D**4 - (D - 2 * t) ** 4) / D,
        "wpl_y_m3": 0.0,
    }
...
def test_catalog_names():
    ...
    for want in ("CHS 139.7x5", "CHS 114.3x4", "CHS 48.3x3.2", "CHS 219.1x8"):
...
def test_unit_mass_hand_calc():
    ...
    cases = [("CHS 139.7x5", 139.7, 5.0), ("CHS 88.9x4", 88.9, 4.0), ("CHS 48.3x3.2", 48.3, 3.2)]
...
def test_ltb_hollow_gate():
    for kind, props in (
        ("circular_hollow", chs_props(139.7, 5)),
