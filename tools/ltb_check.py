...
from tools.eurocode_params import (
    BETA_LT,
    GAMMA_M1,
    IMPERFECTION_ALPHA,
    LAMBDA_LT_0,
...
from tools.section_data import has_full_dims, it_from_dims, iw_from_dims, read_section_props
...
def c1_from_moment_shape(stations: Sequence[tuple[float, float]]) -> tuple[float, str]:
    ...
    """
    pts = sorted((float(p), float(m)) for p, m in stations if 0.0 <= float(p) <= 1.0)
    if not pts:
...
def chi_lt_reduction(
    alpha_lt: float, lambda_lt: float, lambda_lt0: float = LAMBDA_LT_0, beta: float = BETA_LT
) -> float:
    ...
    phi = 0.5 * (1.0 + float(alpha_lt) * (float(lambda_lt) - lambda_lt0) + beta * lam2)
    chi = 1.0 / (phi + math.sqrt(max(phi * phi - beta * lam2, 0.0)))
    return max(0.0, min(chi, 1.0, 1.0 / lam2))


def mb_rd(chi_lt: float, wy_m3: float, fy_pa: float, gamma_m1: float = GAMMA_M1) -> float:
...
def interaction_annex_b(
    ...
    my_ed: float,
    mb_rd_val: float,  # My,Ed and chi_LT·My,Rk/gamma_M1
    mz_ed: float,
    mz_rk: float,  # Mz,Ed and Wz·fy/gamma_M1
    lambda_y: float,
    lambda_z: float,
    cmy: float = 1.0,
    cmz: float = 1.0,
...
def check_ltb_member(
    ...
    *_source : bracing source tag ("explicit"|"brace_points"|"defaulted")
    ...
    base = {"length_m": round(length, 4), "note": LTB_NOTE, "warnings": warnings}
    ...
    result = {
        **base,
        "applies": True,
        "status": "PASS",
        "section": props.get("name", ""),
        "shape_type": props["shape_type"],
        "classification": cls,
        "c1": round(c1, 4),
        "c1_case": c1_case,
        "lcr_lt_m": round(l_lt, 4),
        "lcr_lt_source": lcr_lt_source,
...
def _interaction(
    ...
    inter = interaction_annex_b(
        abs(axial_n), ncr_y, ncr_z, chi_y, chi_z, nrk, m_ed, mb_rd_val, 0.0, mz_rk, lam_y, lam_z
    )
    result.update(
        {
            "lcr_y_m": round(ly, 4),
            "lcr_y_source": lcr_y_source,
            "lcr_z_m": round(lz, 4),
            "lcr_z_source": lcr_z_source,
            "axial_n_kN": round(axial_n / 1e3, 3),
            "chi_y": round(chi_y, 4),
            "chi_z": round(chi_z, 4),
            "lambda_y": round(lam_y, 4),
            "lambda_z": round(lam_z, 4),
            "ncr_y_kN": round(ncr_y / 1e3, 2),
            "ncr_z_kN": round(ncr_z / 1e3, 2),
            "eq61": round(inter["eq61"], 4),
            "eq62": round(inter["eq62"], 4),
            "kyy": round(inter["kyy"], 4),
            "kzz": round(inter["kzz"], 4),
        }
    )
    util = max(util_ltb, inter["eq61"], inter["eq62"])
    result["utilization"] = round(util, 4)
    result["governing"] = (
        ("eq62" if inter["eq62"] >= inter["eq61"] else "eq61") if util > util_ltb + 1e-9 else "ltb"
    )
    if util > 1.0:
        result["status"] = "FAIL"
        result["reason"] = f"governing utilization {util:.2f} > 1.0 ({result['governing']})."
    return result
...
def _check_one_bar(
    bridge, bar_id: int, sec_name: str, length_m: float, df, case_id: int
) -> dict[str, Any]:
    """Gathers live section / material / bracing / forces for one bar and
    runs the pure check_ltb_member."""
    base = {"bar_id": int(bar_id)}
    if not sec_name:
        return {**base, "status": "NOT_CHECKABLE", "reason": "bar has no section assigned."}
    ...
    try:
        ...
        return {**base, "status": "NOT_CHECKABLE", "reason": f"material lookup failed: {exc}"}
    fy_mpa, fy_source = effective_yield_strength(re_mpa, mat_name, props["tf_m"] * 1000.0)
    if fy_mpa is None:
        return {
            **base,
            "status": "NOT_CHECKABLE",
    ...
    result = check_ltb_member(
        ...
        stations,
        axial_n=axial_n,
        lcr_lt=resolved.get("lcr_lt_m"),
        lcr_lt_source=resolved.get("lcr_lt_source"),
        lcr_y=resolved.get("lcr_y_m"),
        lcr_y_source=resolved.get("lcr_y_source"),
        lcr_z=resolved.get("lcr_z_m"),
        lcr_z_source=resolved.get("lcr_z_source"),
        warnings=warnings,
    )
...
def check_lateral_torsional_buckling(
    ...
    for b in ids:
        if b not in real_bars:
            raise ValueError(
                f"bar {b} does not exist in the model (real bars: {sorted(real_bars)[:10]})."
            )

    df = bridge.export_all_member_forces(case_id=int(case_id), divisions=max(2, int(divisions)))
    ...
    for bar_id in ids:
        ...
        try:
            ...
            sec_name = ""
        rows.append(_check_one_bar(bridge, bar_id, sec_name, length_m, df, int(case_id)))
