...
from tools.section_sizing import check_section_proportions, section_depth_mm, suggest_section
...
def _legacy_truss_spec(span, height, panels, top_section, bottom_section, web_section):
    ...
    return {
        ...
        "nodes": nodes,
        "bars": bars,
        "supports": [{"node": bot[0], "type": "pinned"}, {"node": bot[-1], "type": "pinned"}],
        "__tpl": "truss",
    }


def _legacy_tank_spec(radius, height, segments, ring_levels, section_vertical, section_ring):
...
def _legacy_arch_truss_spec(
    span, rise, panels, top_section, bottom_section, web_section, arch_chord
):
...
def test_connect_chords_pratt():
    ...
    bars = connect_chords(
        ...
        "L 50x50x5",
        pattern="pratt",
        chord_a_section="IPE 200",
        chord_b_section="IPE 100",
    )
...
def test_connect_chords_errors():
    ...
    _expect_value_error(lambda: connect_chords([1, 2, 3], [4, 5, 6], "IPE 200", pattern="howe"))
    print("  OK: connect_chords error handling")


def test_radial_ring():
    nodes = radial_ring(
        lambda ratio: (0.0, 0.0, round(5.0 * ratio, 6)), lambda ratio: 2.5, 16, 3, start_id=1
    )
...
def test_legacy_byte_identity():
    ...
    for c in arch_cases:
        ...
        assert json.dumps(new) == json.dumps(old), (
            f"arch_truss_spec mismatch for {c[:4]} arch={c[6]}"
        )
    print(
        f"  OK: truss + tank + arch_truss specs byte-identical to legacy "
...
def test_suggest_section():
    ...
    assert section_depth_mm(suggest_section("beam", 30.0)) > section_depth_mm(
        suggest_section("beam", 1.0)
    )
    notes = []
    ...
    assert section_depth_mm(suggest_section("column", 6.0)) > section_depth_mm(
        suggest_section("column", 3.5)
    )
    ...
    print("  OK: suggest_section sizing / nearest-catalog / notes")
    print(
        f"      acceptance: beam 1m -> {suggest_section('beam', 1.0)}  |  "
...
def test_template_auto_sizing():
    ...
    print(
        f"      acceptance: truss_spec(span=1.0) chord -> {small['bars'][0]['section']}"
        f"  |  truss_spec(span=30.0) chord -> {big['bars'][0]['section']}"
    )
    assert small["bars"][0]["section"] != "IPE 200", "1m truss must not get the old fixed 'IPE 200'"
    ...
    explicit = RobotBridge.truss_spec(
        span=1.0, top_section="IPE 500", bottom_section="IPE 400", web_section="L 80x80x8"
    )
...
def test_arch_truss():
    ...
    inv = RobotBridge.arch_truss_spec(span=9.0, rise=2.0, panels=8, arch_chord="bottom")
...
def test_registry_schemas():
    ...
    for tpl in (
        ...
        for key in hardcoded:
            if key in props:
                assert "default" not in props[key], f"{tpl}.{key} still hardcodes a section default"
    ...
    for meth, param_names in [
        ...
        ("_tool_create_braced_frame", ("column_section", "beam_section", "brace_section")),
        (
            "_tool_create_rectangular_grid_frame",
            ("column_section", "beam_x_section", "beam_y_section"),
        ),
        ("_tool_create_cylindrical_tank", ("section_vertical", "section_ring")),
        ("_tool_create_arch_truss", ("top_section", "bottom_section", "web_section")),
...
def test_compose_chord_generators():
    from tools.geometry_primitives import generate_arc_chord, generate_straight_chord

    c = generate_straight_chord(30.0, 10, elevation=0.0, plane=6.0, section="IPE 500", start_id=1)
...
def test_compose_web_and_bracing_primitives():
    ...
    b2 = generate_arc_chord(30.0, 5.0, 10, plane=6.0, section="IPE 500", start_id=100)
    br = connect_bracing(top, b2, "cross", section="L 50x50x5", start_id=500)
...
def test_compose_copy_no_id_collision():
    ...
    t._compose_apply_step({"op": "copy", "source": "deck_a", "name": "deck_b", "y_shift": 6})
    ...
    t._compose_apply_step({"op": "copy", "source": "arch_a", "name": "arch_b", "y_shift": 6})
    ...
    for b in all_bars:
        assert b["n1"] in node_set and b["n2"] in node_set, f"bar {b['id']} references missing node"
...
def test_compose_bracing_lengths_sane():
    ...
    from tools.geometry_primitives import connect_bracing, generate_arc_chord
    ...
    b = generate_arc_chord(24.0, 3.0, 10, plane=6.0, section="IPE 500", start_id=100)
    ...
    assert any(y_gap + 0.1 < L for L in lengths), (
        "expected at least one cross brace to span a horizontal/vertical gap"
    )
    print(
        "  OK: bracing lengths = true 3D endpoint distance, physically sane "
...
def test_merge_coincident_nodes():
    ...
    g = {
        ...
        "supports": [{"node": 1, "type": "pinned"}, {"node": 62, "type": "pinned"}],
        "loads": [{"kind": "nodal", "node": 56, "case": 1, "fz": -10.0}],
    }
...
def test_compose_coincident_node_detector():
    ...
    b._node_coords = {n: ((n % 5) * 3.0, (n // 5) * 6.0, (n % 2) * 2.0) for n in range(1, 12)}
    ...
    assert b._coincident_node_pairs() == []
    print(
        "  OK: coincident-node detector flags twin-arch springing, "
...
def test_compose_per_op_validation():
    ...
    def _expect_error(step, frag, executor=None):
        ...
        try:
            ...
            assert frag.lower() in str(exc).lower(), f"expected {frag!r} in: {exc}"
            return
        raise AssertionError(f"expected ToolExecutionError for step {step}")

    _expect_error(
        {"op": "web", "top": "nope", "bottom": "deck_a", "pattern": "pratt"}, "unknown chain"
    )
    _expect_error({"op": "copy", "source": "ghost", "name": "g", "y_shift": 6}, "unknown chain")
    _expect_error(
        {"op": "copy", "source": "deck_a", "name": "deck_b", "y_shift": "six"},
        "y_shift must be a number",
    )
    _expect_error(
        {"op": "chord", "name": "bad_span", "kind": "straight", "span": -5, "n_panels": 4},
        "span must be > 0",
    )
    _expect_error(
        {"op": "chord", "name": "bad_panels", "kind": "straight", "span": 10, "n_panels": 1},
        "n_panels must be >= 2",
    )
    ...
    _expect_error(
        {"op": "bracing", "plane_a": "arch_a", "plane_b": "deck_a", "pattern": "cross"},
        "different panel counts",
        executor=t,
    )
...
def test_compose_copy_then_bracing_no_id_collision():
    ...
    assert len(bar_ids) == len(set(bar_ids)), "duplicate bar ids after copy->bracing: " + str(
        sorted({i for i in bar_ids if bar_ids.count(i) > 1})
    )
    print(f"  OK: copy-then-bracing never collides bar ids ({len(bar_ids)} unique bars)")
...
def test_compose_full_assembly_finish():
    ...
    print(f"  OK: full twin-arch compose -> finish() spec integrity clean ({res['counts']})")
