...
def worst_of(per: dict[str, dict[str, Any]]) -> tuple:
    ...
    ranked = [(ch, _STATUS_RANK.get(per[ch]["status"], -1)) for ch in _CHECK_ORDER if ch in per]
    worst_check, worst_rank = max(ranked, key=lambda t: (t[1], -_CHECK_ORDER.index(t[0])))
    overall = "FAIL" if worst_rank == 2 else ("NOT_CHECKABLE" if worst_rank == 1 else "PASS")
...
def check_eurocode_members(
    ...
    for bar_id in ids:
        ...
        if axial_kN >= 0.0:
            continue
        try:
            res = check_euler_buckling(bridge, bar_id, int(case_id), axial_force_kn=axial_kN)
            euler_by_bar[bar_id] = {
    ...
    for conn in bridge.connections.all_connections():
        ...
        if bar_id not in ids:
            continue
        try:
            res = bridge.check_connection_capacity(bar_id, conn["joint_end"], case_id=int(case_id))
            conn_by_bar[bar_id] = {
    ...
    for bar_id in ids:
        ...
        per = {
            "elastic": {"status": u.get("Status", "N/A"), "utilization": u.get("Utilization")},
            "buckling": {"status": e.get("status", "N/A"), "utilization": e.get("utilization")},
            "ltb": {
