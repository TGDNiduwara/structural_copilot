...
BOLT_GRADES: Dict[str, float] = {
    "4.6": 400.0, "5.6": 500.0, "8.8": 800.0, "10.9": 1000.0,
}
#: Table 3.4 alpha_v per bolt class.
ALPHA_V: Dict[str, float] = {
    "4.6": 0.6, "5.6": 0.6, "8.8": 0.6,
    "4.8": 0.5, "5.8": 0.5, "6.8": 0.5, "10.9": 0.5,
}
...
class ConnectionRegistry:
    ...
    def set_connection(self, bar_id: int, joint_end: str = "end",
                       **kwargs) -> Dict[str, Any]:
        bar_id = int(bar_id)
        ...
        if conn["bolt_grade"] not in BOLT_GRADES:
            raise ValueError(
                f"bolt_grade must be one of {sorted(BOLT_GRADES)}, got "
                f"'{conn['bolt_grade']}'")
    ...
    def get(self, bar_id: int, joint_end: str = "end") -> Optional[Dict[str, Any]]:
        entry = self._entries.get(int(bar_id), {}).get(
            str(joint_end or "end").lower())
...
def bolt_shear_resistance(bolt_grade: str, d_mm: float, n_bolts: int,
                          planes: int = 1,
                          gamma_m2: float = GAMMA_M2) -> float:
...
def bearing_resistance(fu_mpa: float, d_mm: float, t_mm: float,
                       e1_mm: float, e2_mm: float, p1_mm: float,
                       p2_mm: float, n_bolts: int, fub_mpa: float,
                       gamma_m2: float = GAMMA_M2) -> float:
    ...
    per_bolt = k1 * alpha_b * float(fu_mpa) * 1e6 * float(d_mm) * 1e-3 \
        * float(t_mm) * 1e-3 / gamma_m2
    return per_bolt * max(int(n_bolts), 1)


def block_shear_resistance(fy_mpa: float, fu_mpa: float, t_mm: float,
                           e1_mm: float, p1_mm: float, n_bolts: int,
                           d_mm: float, gamma_m2: float = GAMMA_M2,
                           gamma_m0: float = GAMMA_M0) -> float:
    ...
    anv = 2.0 * float(t_mm) * (float(n_bolts) - 1.0) * float(p1_mm) \
        + 2.0 * float(t_mm) * float(e1_mm) - float(n_bolts) * float(t_mm) * d0
    # MPa * mm2 = N — no further scaling.
    return (float(fu_mpa) * ant / gamma_m2
            + float(fy_mpa) * anv / (math.sqrt(3.0) * gamma_m0))


def weld_resistance(fu_mpa: float, weld_leg_mm: float, weld_length_mm: float,
...
def check_simple_shear_connection(
    conn: Dict[str, Any],
    v_ed_n: float,
    member_fu_mpa: Optional[float] = None,
    member_web_t_mm: Optional[float] = None,
) -> Dict[str, Any]:
    ...
    if conn.get("bolt_grade") not in BOLT_GRADES:
        return {"status": "NOT_CHECKABLE",
                "reason": "no usable connection definition — a valid "
                          "bolt_grade is required.",
                "note": CONNECTION_NOTE}
    fu_plate = fu_for_grade(plate_grade) or 430.0
    fy_plate = fy_for_grade(plate_grade,
                            conn.get("plate_thickness_mm", 10.0)) or 275.0
    ...
    if conn.get("bolt_grade") in BOLT_GRADES:
        add("bolt_shear",
            bolt_shear_resistance(conn["bolt_grade"], d, n, planes))
    add("bearing_plate",
        bearing_resistance(fu_plate, d, t, e1, e2, p1, p2, n, fub))
    if member_fu_mpa and member_web_t_mm:
        add("bearing_web",
            bearing_resistance(member_fu_mpa, d, float(member_web_t_mm),
                               e1, e2, p1, p2, n, fub))
    add("block_shear",
        block_shear_resistance(fy_plate, fu_plate, t, e1, p1, n, d))
    weld_leg = conn.get("weld_leg_mm")
    if weld_leg:
        weld_len = (n - 1) * p1 + 2.0 * e1
        add("weld",
            weld_resistance(fu_plate, float(weld_leg), weld_len, plate_grade))
    ...
    return {
        ...
        "plate_grade": plate_grade, "bolt_grade": conn["bolt_grade"],
        "bolt_diameter_mm": d, "bolts": n,
        "note": CONNECTION_NOTE,
    }
