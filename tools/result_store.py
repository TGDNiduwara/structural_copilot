...
def _df_to_markdown(df: Optional[pd.DataFrame], max_rows: int = 12) -> str:
...
class ResultStore:
    ...
    def store(
        ...
        member_forces: Optional[pd.DataFrame],
        ...
        if boq is not None and not boq.empty and "Total_Weight_kg" in boq.columns:
            try:
                total_weight = float(pd.to_numeric(
                    boq["Total_Weight_kg"], errors="coerce").sum())
            except Exception:
                total_weight = None
        max_my = None
        if member_forces is not None and not member_forces.empty \
        ...
        if max_util is not None:
            bits.append(f"max utilization {max_util:.2f} ({gov_check}); "
                        f"{n_pass} pass / {n_fail} fail"
                        + (f" / {n_uncheckable} not checkable"
                           if n_uncheckable else ""))
        return ", ".join(bits) + "."
    ...
    def retrieve(self, key: str) -> str:
        ...
        if snap is None:
            ...
            return (f"Error: no stored result named '{key}'. "
    ...
    def list_results(self) -> str:
        ...
        if not self._snapshots:
            return ("No stored results yet. Build a model, run solve + the "
