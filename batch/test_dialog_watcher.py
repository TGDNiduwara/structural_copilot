...
from batch.headless_driver import (HeadlessSession, SolverInstabilityError,
                                   UnknownDialogError, _robot_pids)
...
with HeadlessSession(visible=False) as s:
    check("(c) Interactive=0 set by connect()",
          int(s.bridge.robot_app.Interactive) == 0)
...
try:
    ...
    check("(a) clean instability failure", "instability" in str(exc).lower(),
          str(exc)[:130])
except Exception as exc:  # noqa: BLE001
    check("(a) expected SolverInstabilityError", False,
...
try:
    ...
    check("(a) same session solves valid model after",
          r["static"]["status"] == "ok")
except Exception as exc:  # noqa: BLE001
    check("(a) same session solves valid model after", False,
          f"{type(exc).__name__}: {exc}")
pid_after = sorted(s._owned_pids)
check("(a) no relaunch (same PID)", pid_before == pid_after,
      f"{pid_before} vs {pid_after}")
s.close()
...
try:
    ...
    check("(b) unknown fallback (kill + logged title)",
          "instability" in str(exc).lower(), str(exc)[:150])
except Exception as exc:  # noqa: BLE001
    check("(b) expected UnknownDialogError", False,
          f"{type(exc).__name__}: {exc}")
time.sleep(2)
now = len(_robot_pids())
check("(b) unknown-dialog process force-killed", now == base_count,
      f"now={now} base={base_count}")
s2.close()
