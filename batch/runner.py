...
from batch.buckling_check import EULER_NOTE, check_euler_buckling
...
_SOLVER_SAFETY_ERRORS = (SolverInstabilityError, UnknownDialogError, TimeoutError)
...
def _buckling_for_candidate(
    ...
    worst: dict[str, Any] | None = None  # worst = smallest margin Pcr/N
    for row in per_bar:
    ...
    detail = (
        f"worst bar {worst['bar_id']} ({worst.get('section')}): "
...
def _evaluate_candidate(
    ...
    buckling_status, buckling_pass = _buckling_for_candidate(session.bridge, util, eval_case_id)
    ...
    if want_ltb:
        try:
            ltb_res = check_lateral_torsional_buckling(session.bridge, eval_case_id)
            statuses = [r.get("status") for r in ltb_res.get("bars", [])]
            ltb_status = (
                "FAIL"
                if "FAIL" in statuses
                else ("NOT_CHECKABLE" if "NOT_CHECKABLE" in statuses else "PASS")
            )
...
def _load_pending(storage: Storage, run_id: int, resume_index: int | None) -> list[dict[str, Any]]:
    ...
    for _, row in df.iterrows():
        ...
        if resume_index is not None and idx <= resume_index:
            continue
        pending.append({"candidate_id": cid, "candidate_index": idx, "design_vars": dv})
...
def run_batch(
    ...
    session_factory: Callable[[], HeadlessSession] | None = None,
) -> dict[str, Any]:
    ...
    if run_id is None:
        ...
        logger.info("Run %s created with %d candidates.", run_id, len(candidates))
    else:
    ...
    if resume_index is not None:
        logger.info("Resume point: %s (skipping candidates <= it).", resume_index)
    ...
    session: HeadlessSession | None = factory()
    session.connect()
    ...
    try:
        for i, cand in enumerate(pending, start=1):
            ...
            t_c = time.time()
            try:
                result = _evaluate_candidate(session, design_space, cand["design_vars"], eval_case)
            ...
            eta = avg * (total - i)
            logger.info(
                "  checkpoint %s after %.1fs | [%d/%d] run elapsed %.0fs, ETA ~%.0fs",
                idx,
                elapsed,
                i,
                total,
            ...
            if storage.is_cancel_requested(run_id):
                logger.warning("Run %s cancellation requested - stopping", run_id)
    ...
    logger.info(
        "Run %s completed: %d evaluated, %d failed in %.1fs.",
        run_id,
        n_evaluated,
        n_failed,
        time.time() - t_start,
