...
class _FakeSession:
    ...
    def export_all_member_forces(self, case_id=1, divisions=2):
        return pd.DataFrame(
            [
                {"Bar_ID": bid, "Position_m": k * 1.0 / max(divisions - 1, 1), "FX_kN": 50.0}
                for bid in self._sections
    ...
    def _response(self):
        ...
        worst = max((1.15 - 0.0020 * _size_of(sec)) for sec in self._sections.values()) or 1.15
        return weight, worst


def _load_spec():
    return {"cases": [{"id": 1, "name": "DL", "nature": "permanent"}], "loads": []}
...
def test_compare_topologies_fake_flow():
    ...
    variants = [
        {
            "name": "truss",
            "generator": "create_truss",
            "generator_args": {"span": 6.0, "height": 1.5, "panels": 3},
        },
        {
            "name": "frame",
            "generator": "create_braced_frame",
            "generator_args": {"height": 3.0, "width": 6.0},
        },
    ]
    result = compare_topologies(
        variants, _load_spec(), db_path=db, log_path=log, session_factory=factory
    )
    ...
    print(
        f"TOPOLOGY COMPARE: {len(ranked)} variants sized via real "
