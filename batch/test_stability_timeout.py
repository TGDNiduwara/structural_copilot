...
with HeadlessSession(visible=False) as s:
    ...
    st = s.validate_stability()
    check("S2b fixed column validate ok (mathematically stable)",
          bool(st.get("ok")), st["message"])
    r = s.solve_all(["static"])
...
with HeadlessSession(visible=False) as s:
    ...
    st = s.validate_stability()
    check("S2c pinned+lateral mechanism flagged",
          (not st.get("ok")) and st.get("mechanism"), st["message"])
    t0 = time.time()
    try:
        s.solve_all(["static"])
        ...
        dt = time.time() - t0
        check("S2c MechanismError raised fast (no Calculate)",
              dt < 5.0, f"{dt:.2f}s: {exc}")
...
post = robot_count()
check("S3 force-kill left no orphaned robot.exe", post == base_count,
