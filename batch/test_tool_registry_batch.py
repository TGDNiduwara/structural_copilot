...
SMALL_SPEC = {
    ...
    "variable_groups": [
        {"group_name": "beam", "bar_ids": [1],
         "candidate_sections": ["IPE 300", "IPE 360"]},
    ],
}
...
LARGE_SPEC = {
    ...
    "variable_groups": [
        {"group_name": "columns", "bar_ids": [1, 3],
         "candidate_sections": ["HEA 160", "HEA 180", "HEA 200", "HEA 220",
                                "HEA 240", "HEB 160", "HEB 180", "HEB 200"]},
        {"group_name": "beam", "bar_ids": [2],
         "candidate_sections": ["IPE 200", "IPE 220", "IPE 240", "IPE 270",
                                "IPE 300", "IPE 330", "IPE 360", "IPE 400"]},
    ],
}

def test_schemas_and_handlers_registered():
    names = {s["name"] for s in TOOL_SCHEMAS}
    for name in ("start_surrogate_search_run",
                 "confirm_and_start_surrogate_search_run",
                 "export_best_design"):
        assert name in names, f"schema missing: {name}"
    assert ".rtd" in ALLOWED_EXTENSIONS
    ex = ToolExecutor()
    for attr in ("_tool_start_surrogate_search_run",
                 "_tool_confirm_and_start_surrogate_search_run",
                 "_tool_export_best_design", "_run_surrogate_worker"):
        assert hasattr(ex, attr), attr
    _validate_tool_arguments("start_surrogate_search_run",
                             {"spec": LARGE_SPEC, "budget": 40,
                              "patience": 5, "acquisition": "ehvi",
                              "kappa": 1.5})
    _validate_tool_arguments("confirm_and_start_surrogate_search_run",
                             {"run_config_id": "surr_cfg_1"})
    _validate_tool_arguments("export_best_design",
                             {"run_id": 3, "file_name": "winner.rtd",
                              "frontier_index": 0})
...
def test_start_small_grid_recommends_grid():
    ...
    assert not ex._optimization_configs, \
        "grid_recommended must not stage a config"
    print("START: small grid -> grid_recommended, nothing staged")


def test_start_large_stages_and_confirm_guard():
    ex = ToolExecutor()
    r = ex._tool_start_surrogate_search_run(LARGE_SPEC, budget=25,
                                            patience=3, acquisition="ucb")
    ...
    print("START+CONFIRM: staged surrogate config; bogus/grid/surrogate "
          "cross-start guards all reject")
...
def test_export_best_design_not_ready_and_missing():
    ...
    run_id = st.create_run({"geometry": {}, "variable_groups": []},
                           objective="")
    ...
    print("EXPORT: non-completed run -> not_ready (no Robot); missing run "
          "raises cleanly")
