...
class HeadlessSession:
    ...
    def validate_stability(self) -> Dict[str, Any]:
        """
        [STEP 2] Detects likely kinematic mechanisms BEFORE Calculate() is
        ever called, so the solver's instability modal (which Interactive=0
        suppresses but cannot answer) is never triggered.

        Uses a small 2D Euler-Bernoulli frame stiffness assembly (3 DOF per
        node: UX, UZ, RY): the global stiffness matrix is reduced by the
        support fixity flags and its rank checked. A rank-deficient matrix
        means a mechanism; the nullspace identifies the node/DOF involved.

        NOTE: linear-elastic kinematics check, not a stability (eigenvalue)
        analysis. It catches mechanisms Robot would flag; the solve()
        timeout (Step 3) is the backstop.
        """
        bridge = self.bridge
        st = bridge.structure
        ...
        for i in range(1, n_count + 1):  # collection Get() is 1-based
            try:
                ...
                continue
        if not node_ids:
            return {"ok": True, "mechanism": False,
                    "nodes": [], "dofs": [], "message": "no nodes in model"}
        n = len(node_ids)
        idx = {nid: k for k, nid in enumerate(node_ids)}

        # --- supports: node -> (UX, UZ, RY) fixity ---
        ...
        for i in range(1, b_count + 1):
            try:
                ...
                bars.append((idx[n1], idx[n2], a, ii))
            except Exception:  # noqa: BLE001
                continue

        # --- assemble 3n x 3n global stiffness (2D Euler-Bernoulli) ---
        k = np.zeros((3 * n, 3 * n))
        for i1, i2, a, ii in bars:
            (x1, z1), (x2, z2) = coords[node_ids[i1]], coords[node_ids[i2]]
            dx, dz = x2 - x1, z2 - z1
            length = float(np.hypot(dx, dz))
            if length < 1e-12:
                continue
            c, s = dx / length, dz / length
            ea, ei = 1.0 * a, 1.0 * ii  # E cancels for rank purposes
            k_local = np.array([
            ...
            dofs = []
            for i_node in (i1, i2):
                dofs += [3 * i_node, 3 * i_node + 1, 3 * i_node + 2]
            for r, dr in enumerate(dofs):
                for c_, dc in enumerate(dofs):
                    k[dr, dc] += k_e[r, c_]

        # --- apply supports: drop fixed DOFs, check rank of free system ---
        free = []
        for nid in node_ids:
        ...
        # rank-deficient -> find nodes whose DOFs participate in the nullspace
        ...
        for j, dof in enumerate(free):
            if mag > 0 and abs(null[j]) > 0.25 * mag:
                node = node_ids[dof // 3]
                dofs.append(f"node {node} {_DOF_NAMES_2D[dof % 3]}")
                if node not in flagged:
                    flagged.append(node)
        return {
            "ok": False, "mechanism": True,
            "nodes": flagged,
            "dofs": dofs[:8],
