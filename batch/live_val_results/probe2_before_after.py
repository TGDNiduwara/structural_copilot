"""probe2_before_after.py - does the app-path solve keep the model & loads?
Builds the same truss + self-weight, snapshots bar count / case records /
member-force values BEFORE and AFTER solve(), and reports whether the
model or the results are lost by the interactive save-changes dialog."""
...
def case_records(b, case_id=1):
    try:
        case = b.structure.Cases.Get(case_id)
        sc = CastTo(case, "IRobotSimpleCase")
        return int(sc.Records.Count)
...
print(ex.dispatch("create_truss", {"span": 20.0, "height": 3.0, "panels": 6,
                                   "web_section": "L 50x50x5"})[:150])
ex.dispatch("create_load_case", {"case_id": 1, "case_name": "SW",
                                 "nature": "permanent"})
print(ex.dispatch("apply_self_weight", {"case_id": 1})[:200])

print("\n== BEFORE solve ==")
print("bars:", bar_count(b))
print("case1 records:", case_records(b))

print("\n== solve ==")
print(ex.dispatch("solve", {"timeout_s": 120})[:180])
