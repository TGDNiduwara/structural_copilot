...
try:
    ...
    check("'IPE  chord' rejected", "placeholder" in str(e).lower(), f"-> {str(e)[:80]}")
...
check("'L 120x120x5' accepted", RobotBridge._validate_section_input("L 120x120x5") == "L 120x120x5")
try:
...
for want in ("L 120x120x5", "L 120X120X5", "L 120 x 120 x 5", "L120x120x5", "L120X120X5"):
...
check("duplicate node id detected", any("duplicate node" in i for i in issues), str(issues))
bad_bar = {
    "nodes": [{"id": 1, "x": 0, "z": 0}, {"id": 2, "x": 1, "z": 0}],
...
check("list_available_sections guidance present", "list_available_sections" in desc)
...
check(
    "angle family returns RESOLVABLE names",
    "L 120x120x10" in ang and "L 50x50x5" in ang,
    f"(n={len(ang)})",
)
check(
    "no bare leg sizes (the catalog-miss root cause)",
...
check(
    "L-family guidance in description",
    "leg" in las.lower() and "x" in las,
...
try:
    ...
    check(
        "claim records ownership",
        c1["owner_pid"] == 1111 and c1["owner_kind"] == "batch" and c1["robots_alive"] is not None,
    )
    try:
    ...
    try:
        ...
        check("empty robot_pids claim REFUSED", "at least one real robot.exe pid" in str(exc))
    check(
        "malformed seat never written",
        rs.seat_status().get("owner_pid") != 1111 or rs.seat_status().get("robot_pids"),
    )
