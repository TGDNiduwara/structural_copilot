...
try:
    ...
    check("create_run returns int id", isinstance(run_id, int) and run_id >= 1,
          f"run_id={run_id}")

    candidate_ids = []
    for i in range(10):
        cid = s.add_candidate(
            run_id, {"beam": f"IPE{300 + i}", "columns": "HEA220"})
    ...
    s.close()
    check("connection closed (restart simulated)",
          s._conn is None, "")
    ...
    check("columns present",
          {"candidate_id", "weight_kg", "max_utilization",
           "governing_check", "buckling_status", "pass_fail",
           "design_vars_json", "evaluated_at"} <= set(df.columns),
          str(list(df.columns)))
    ...
    check("mark_run_status completed",
          s2.get_run(run_id)["status"] == "completed")
