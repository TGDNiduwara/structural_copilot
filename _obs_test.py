...
from agent.llm_providers import call_llm, LLMProviderError, API_KEY_ENV_VARS  # noqa: E402
...
def _run_turn(ex: ToolExecutor, messages, max_steps: int) -> dict:
    ...
    for step in range(max_steps):
        response = call_llm(
            ...
            messages=messages, tool_schemas=ex.get_tool_schemas(),
            temperature=0.2, base_url=None, attachments=None)
    ...
    lifecycle += ex.drain_activity()
    return {"reasoning": reasoning, "tool_names": tool_names,
            "lifecycle": lifecycle, "final": final_text}
...
def main() -> int:
    ...
    if len(connected) < 2:
        ...
        print(f"VERDICT: SAME PID ({connected[0]}) across all turns - Robot was "
        ...
        print(f"VERDICT: PID CHANGED across turns ({sorted(set(connected))}) - "
